...
export default function AuthPage() {
  ...
  // OAuth callbacks (Google via hash, GitHub via query params)
  useEffect(() => {
    // Google: session_id comes in hash fragment
    ...
    // Google/GitHub: session_id or code in query params
  ...
  const githubLogin = async () => {
    try {
      ...
      const res = await api.githubAuthUrl(redirectUri);
      window.location.href = res.url;
    } catch (err) {
      if (err.message.includes('not configured')) {
        setError('GitHub login is not configured yet. Contact admin.');
      } else {
        setError(err.message);
      }
    }
        ...
        {(mode === 'login' || mode === 'register') && (
          <>
            ...
            <button onClick={googleLogin} data-testid="google-login-btn"
              className="w-full flex items-center justify-center gap-3 py-3 mb-2 bg-white/[0.03] border border-white/10 hover:border-white/20 transition-all font-ui text-sm text-white/70 hover:text-white animate-fade-in-up"
            ...
            {/* GitHub Login */}
            <button onClick={githubLogin} data-testid="github-login-btn"
