...
export default function Sidebar({ activePage, setActivePage, onSearch }) {
  ...
  return (
    <div className={`h-screen flex flex-col border-r border-white/10 bg-[#050505] transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'}`}
      ...
      <div className="p-4 flex items-center justify-between border-b border-white/10">
        ...
        <button onClick={() => setCollapsed(!collapsed)} className="text-white/40 hover:text-white transition-colors p-1"
        ...
        <form onSubmit={handleSearch} className="p-3 border-b border-white/10">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search..."
              className="w-full bg-white/5 border border-white/10 pl-9 pr-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[var(--accent-color)] transition-colors font-body"
            />
          </div>
          <div className="flex gap-1 mt-2">
            {['all', 'files', 'accounts'].map(t => (
              <button key={t} type="button"
                ...
                className={`flex-1 text-[10px] uppercase tracking-widest py-1 font-ui transition-all ${
                  searchType === t
      ...
      {/* User */}
      <div className="p-3 border-t border-white/10 relative" ref={dropdownRef}>
        <button
          data-testid="user-menu-button"
          onClick={() => setDropdownOpen(!dropdownOpen)}
          className="w-full flex items-center gap-3 p-2 hover:bg-white/5 transition-colors"
        >
          <img src={user?.avatar_url} alt="" className="w-8 h-8 rounded-sm bg-white/10" />
          {!collapsed && (
            <>
        ...
        {dropdownOpen && (
          <div className="absolute bottom-full left-3 right-3 mb-2 bg-[#0A0A0A] border border-white/10 animate-fade-in-up z-50"
