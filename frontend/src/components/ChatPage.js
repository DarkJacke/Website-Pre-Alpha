        ...
        <div className="flex-1 overflow-y-auto">
          {chats.length === 0 ? (
            <div className="p-6 text-center text-white/30 font-ui text-sm" data-testid="no-chats">
              ...
              <p className="text-[10px] mt-1 text-white/20">Search for users to start chatting</p>
            </div>
          ) : (
            chats.map(chat => (
              <button key={chat.chat_id}
                ...
                className={`w-full flex items-center gap-3 p-3 md:p-4 text-left transition-colors border-b border-white/5 ${
                  activeChat?.chat_id === chat.chat_id ? 'bg-white/5' : 'hover:bg-white/5'
                }`}
              >
                <img src={chat.other_user?.avatar_url || ''} alt=""
                  className="w-9 h-9 md:w-10 md:h-10 rounded-sm bg-white/10 shrink-0" />
      ...
      {activeChat ? (
        <div className="flex-1 flex flex-col min-w-0">
          <div className="p-3 md:p-4 border-b border-white/10 flex items-center gap-3 bg-[#050505] shrink-0">
            <button onClick={() => setActiveChat(null)} className="text-white/50 hover:text-white md:hidden"
              data-testid="chat-back-btn">
              <ArrowLeft size={18} />
            </button>
            <img src={activeChat.other_user?.avatar_url || ''} alt=""
              className="w-8 h-8 rounded-sm bg-white/10 shrink-0" />
          ...
          <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-2.5" data-testid="chat-messages">
            {messages.map((msg, i) => {
              ...
              return (
                ...
                >
                  <div className={`max-w-[80%] md:max-w-[70%] px-3.5 py-2.5 ${
          ...
          <form onSubmit={sendMessage} className="p-3 md:p-4 border-t border-white/10 flex gap-2 md:gap-3 bg-[#050505] shrink-0 safe-bottom">
            <input
              ...
              maxLength={5000}
              className="flex-1 bg-white/5 border border-white/10 px-3 md:px-4 py-2.5 md:py-3 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-[var(--accent-color)] transition-colors font-body min-w-0"
            />
            <button type="submit" data-testid="chat-send-btn"
              className="px-3 md:px-4 py-2.5 md:py-3 transition-all duration-200 active:scale-95 shrink-0"
